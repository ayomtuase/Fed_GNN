...
Extracts numeric traffic features for temporal, content, and behavioral attack detection.
"""
...
class FeatureEngineer:
    ...
    def extract_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Engineered {len(self.created_features)} new features for {self.detector_type} detection"
        )
        return result_df
    ...
    def _add_base_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "Flow Duration" in df.columns and "Tot Fwd Pkts" in df.columns:
            df["flow_rate"] = (df["Tot Fwd Pkts"] + df["Tot Bwd Pkts"]) / (
                df["Flow Duration"] / 1000000 + 1e-6
            )
    ...
    def _add_content_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "TotLen Fwd Pkts" in df.columns:
            ...
            df["unusual_payload"] = (
                df["TotLen Fwd Pkts"] > mean_payload + 2 * std_payload
            ).astype(int)
    ...
    def _add_behavioral_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "Flow Duration" in df.columns:
            ...
            df["is_short_session"] = (
                df["Flow Duration"] < median_duration / 10
            ).astype(int)
            df["is_long_session"] = (df["Flow Duration"] > median_duration * 10).astype(
                int
            )
